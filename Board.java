...
public class Board {
	...
	private HashMap<Integer, Character> usedTiles;
	...
	private int getTotalWordScore (Word word) {
	...
	public int setWord (Word word, Frame frame) {
		...
		int totalWordScore;
		char currentLetter;
		usedTiles = new HashMap<Integer, Character>();
		...
		for (int i=0; i<word.getLength(); i++) {
			...
			if (word.isHorizontal()) {
				usedTiles.put(column,currentLetter);
				column++;
			}
			else {
				usedTiles.put(row,currentLetter);
				row++;			
			}
		}
		numPlays++;		
	...
	public void removeWord(Word word, Frame frame){
		int row, column;
		...
		for (int i=0; i<word.getLength(); i++) {
			...
			if (word.isHorizontal()) {
				if(usedTiles.containsKey(column)){
					frame.add(usedTiles.get(column));
					usedTiles.remove(column);
					sqContents[row][column] = EMPTY;
				}
				column++;
			}
			else { 
				if(usedTiles.containsKey(row)){
					frame.add(usedTiles.get(row));
					usedTiles.remove(column);
					sqContents[row][column] = EMPTY;
				}
				row++;			
