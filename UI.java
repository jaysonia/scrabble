...
public class UI {
	...
	public void displayHelp () {
		...
		System.out.println("For a blank tile, type the lower case letter that you want to place. For all other tiles and commands, use upper case.");
		System.out.println("For a challenge, type challenge to challenge the previous move. Challenge cannot be done as the first move or after a pass or exhange");
	}
	...
	public int getCommand (Player player) {
		...
		do {
			...
			if ( command.equals("QUIT") || command.equals("quit") ||command.equals("Q") || command.equals("q") ) {
				...
				if(prevCommand.equals("PASS") ||prevCommand.equals("P") || prevCommand.matches("EXCHANGE( )+([A-Z*]){1,7}") ||prevCommand.matches("X( )+([A-Z*]){1,7}") || prevCommand.equals("CHALLENGE") ||prevCommand.equals("C") )
					valid = false;
				else{
					valid = true;
